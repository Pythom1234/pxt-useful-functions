...
namespace useful {
    ...
    //% block="comment $c||(run code $run)" block.loc.cs="komentář $c||(spouštět kód $run)" color=#707070 weight=99 handlerStatement run.shadow="toggleYesNo"
    export function commentSection(c: string, a: Action, run: boolean = true) {
        if (run) {
            a()
        }
    }
